...
/**
 * Utility class containing convenience methods for working with java classpaths.
 */
public final class ClasspathUtils {
   ...
   */
  public static List<URI> buildClasspath() throws IOException {
    return buildClasspath(System.getProperty("java.class.path"));
  }

  /**
   * Returns the classpath as a list of paths.
   *
   * @param classpath as a list of classpath entries separated by the operating system's path
   *     separator character to read from.
   * @return the classpath as a list of paths.
   * @throws java.io.IOException if there is an error writing a jar file.
   */
  public static List<URI> buildClasspath(final String classpath) throws IOException {
    final File jarDir = createTempDir("jars-from-classpath", "");

    final ImmutableList.Builder<URI> classpathBuilder = ImmutableList.builder();
    for (final String entry : classpath.split(File.pathSeparator)) {
      ...
      if (entryFile.isDirectory()) {
        final File jarFile = File.createTempFile(
            entryFile.getName(),
            ClasspathUtils.JAR_EXTENSION,
            jarDir
        );
        ClasspathUtils.makeJarFromDirectory(entryFile, jarFile);
        classpathBuilder.add(jarFile.toURI());
      } else {
        classpathBuilder.add(entryFile.toURI());
      }
    }
  ...
  public static void makeJarFromDirectory(
    ...
    try {
      final JarOutputStream jarOutputStream =
          new JarOutputStream(fileOutputStream, ClasspathUtils.JAR_MANIFEST);
      try {
   ...
   * @throws java.io.IOException if there is an error creating the directory.
   */
